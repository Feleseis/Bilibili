    ...
    $('#up-list').click(function (e) {
        ...
        if (mid) {
            $.get(`/ajax?type=user&mid=${mid}`, function (data, status) {
                let htmlUser = `
                    <li><label>用户 :</label><span>${data['user']['name']}</span></li>
                    <li><label>粉丝 :</label><span>${data['user']['fans']}</span></li>
                    <li><label>播放 :</label><span>${data['user']['playNum']}</span></li>
                    <li><label>经验 :</label><span>${data['user']['current_exp']}</span></li>
                    <li><label>投稿 :</label><span>${data['user']['submit']}</span></li>
                `;
                let htmlTlist = '';

                data['user']['tlist'].map((item) => {
                    const percent = Math.round(Number.parseInt(item['count'])/Number.parseInt(data['user']['submit'])*10000)/100;
                    htmlTlist += `<li><label>${item['name']} :</label><span>${item['count']}(${percent}%)</span></li>`
                });
